...
# Windows Features
$useServerManager = ($env:Path -split ';' | ? { $_.length -gt 0} | ForEach-Object { Join-Path -Path $_ -ChildPath 'servermanagercmd.exe' } | Where-Object { Test-Path -Path $_ -PathType Leaf }) -ne $null
$useWmi = $false
$useOCSetup = $false
if( -not $useServerManager )
{
    ...
    $useOCSetup = ($env:Path -split ';' | ? { $_.length -gt 0} | ForEach-Object { Join-Path -Path $_ -ChildPath 'ocsetup.exe' } | Where-Object { Test-Path -Path $_ -PathType Leaf }) -ne $null
}
